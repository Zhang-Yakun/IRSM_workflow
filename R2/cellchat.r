...
unique(meta$group)
cell.use1 <- rownames(meta)[meta$group == "young"] 
cell.use2 <- rownames(meta)[meta$group == "old"] 
# 按指定的变量提取细胞
...
unique(meta2$cell_type)#看meta中储存的细胞注释信息，稍后用它作为分组依据

#####创建cellchat对象

cellchat1 <- createCellChat(object = data.input1, meta = meta1, group.by = "cell_type")
cellchat2 <- createCellChat(object = data.input2, meta = meta2, group.by = "cell_type")

####分别处理这两组样本

#创建celllchat对象，group.by指定通讯间的对象，用meta中的注释作为分组依据
cellchat1 <- addMeta(cellchat1, meta = meta1)
cellchat1 <- setIdent(cellchat1, ident.use = "cell_type") # set "labels" as default cell identity
groupSize1 <- as.numeric(table(cellchat1@idents)) #每种细胞的细胞数量

cellchat2 <- addMeta(cellchat2, meta = meta2)
cellchat2 <- setIdent(cellchat2, ident.use = "cell_type") # set "labels" as default cell identity
...
saveRDS(cellchat1, file = "cellchat1_young.rds")
saveRDS(cellchat2, file = "cellchat2_old.rds")

####合并两组数据
rm(list=ls()) #清空所有变量
options(stringsAsFactors = F) #输入数据不自动转换成因子（防止数据格式错误）

cellchat1<-readRDS("cellchat1_young.rds")
cellchat1<-updateCellChat(cellchat1)

cellchat2<-readRDS("cellchat2_old.rds")
...
gg1 <- compareInteractions(cellchat, show.legend = T, group = c("young","old"))
gg2 <- compareInteractions(cellchat, show.legend = T, group = c("young","old"), measure = "weight")
gg1 + gg2
...
group.cellType <- c(rep("CD8_T", 4), rep("Monocyte", 4), rep("Cytotoxic_CD8_T", 4))
group.cellType <- factor(group.cellType, levels = c("CD8_T", "Monocyte", "Cytotoxic_CD8_T"))
...
group.cellType <- c(rep("Alveolar_Macrophage", 4), rep("Endothelial_Cell", 4), rep("Luminal_Macrophage", 4),rep("Fibroblast", 4))
group.cellType <- factor(group.cellType, levels = c("Alveolar_Macrophage", "Endothelial_Cell", "Luminal_Macrophage","Fibroblast"))
object.list <- lapply(object.list, function(x) {mergeInteractions(x, group.cellType)})
cellchat <- mergeCellChat(object.list, add.names = names(object.list))
...
par(mfrow = c(1,2), xpd=TRUE)
for (i in 1:length(object.list)) {
  netVisual_circle(object.list[[i]]@net$weight.merged, weight.scale = T, label.edge= T, edge.weight.max = weight.max[3], edge.width.max = 12, title.name = paste0("Number of interactions - ", names(object.list)[i]))
}
...
gg1 <- netAnalysis_signalingChanges_scatter(object.list, idents.use = "Endothelial_Cell")
gg2 <- netAnalysis_signalingChanges_scatter(object.list, idents.use = "Alveolar_Macrophage")
...
#此条形图可以以堆叠模式绘制，也可以不绘制。根据young和 old之间推断网络内的整体信息流的差异
#对重要的信号通路进行排序。红色的顶部信号通路在young中富集，而这些绿色在old中富集。
